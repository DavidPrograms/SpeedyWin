...
echo Exit Instalation [N]
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
choice /n /c YN /m "" & if errorlevel 2 exit)
cls
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Please Read The EULA Before Continuating!!!
EULA.rtf
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo You agree? [Y/N]
choice /n /c YN /m "" & if errorlevel 2 exit)
...
cls
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Now will begin the procedure
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Begin [Any Key]
pause >nul
cls
echo =-=-=-=-=-=-=-=-=-=-=-=-=
echo Tweaking... Please Wait
echo =-=-=-=-=-=-=-=-=-=-=-=-=
powercfg -duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61
powercfg /setactive e9a42b02-d5df-448d-aa00-03f14749eb61

regedit.exe /s "TweaksREG.reg"
...
reg add "HKLM\Software\Policies\Microsoft\Windows\Windows Error Reporting" /v "Disabled" /t REG_DWORD /d "4" /f
reg add "HKLM\SOFTWARE\Microsoft\Windows\Windows Error Reporting" /v "Disabled" /t "REG_DWORD" /d "4" /f
reg add "HKLM\Software\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultConsent" /t REG_DWORD /d "0" /f
reg add "HKLM\Software\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultOverrideBehavior" /t REG_DWORD /d "1" /f
reg add "HKLM\Software\Microsoft\Windows\Windows Error Reporting" /v "DontSendAdditionalData" /t REG_DWORD /d "4" /f
reg add "HKLM\Software\Microsoft\Windows\Windows Error Reporting" /v "LoggingDisabled" /t REG_DWORD /d "4" /f
...
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
winget install Microsoft.GamingApp_8wekyb3d8bbwe
winget install Microsoft.XboxIdentityProvider_8wekyb3d8bbwe
...
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Exit app [Any Key]
pause >nul
cls
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Delete SpeedyWin Temp?
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo Yes [Y]
echo No, Exit App [N]
choice /n /c YN /m "" & if errorlevel 2 exit)
